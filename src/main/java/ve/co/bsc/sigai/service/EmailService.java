...
public class EmailService implements ApplicationContextAware, InitializingBean {
    ...
    public void sendMessage(final String from, final String to, final String subject, final String message) throws MailException {
        ...
        this.executor.submit(new Runnable() {
            ...
            public void run() {
                try {
                    logger.debug(Thread.currentThread().getId() + " intentando enviar mail [" + msg + "]");
                    mailSender.send(msg);
                    logger.debug(Thread.currentThread().getId() + " mail enviado satisfactoriamente [" + msg + "]");
                } catch (Exception e) {
                    logger.error("No se pudo enviar mensaje [" + msg + "]", e);
                }
            }
        });
